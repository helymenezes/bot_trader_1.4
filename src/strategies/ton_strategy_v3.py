...
def getAdvancedTradeStrategy_v3(
    ...
    # Verifica se há pelo menos 3 registros antes de acessar o terceiro
    has_third_record = len(df) >= 3
    prev_prev = df.iloc[-3] if has_third_record else None
    ...
    def print_details():
        ...
        # Modificar a impressão de MCAD para lidar com o caso em que prev_prev é None
        if has_third_record:
            print(f" | MCAD(): {latest['MCAD']:.4f} (Anterior: {prev['MCAD']:.4f} (Anterior: {prev_prev['MCAD']:.4f})")
        else:
            print(f" | MCAD(): {latest['MCAD']:.4f} (Anterior: {prev['MCAD']:.4f})")
